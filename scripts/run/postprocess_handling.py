import glob
import os
from run_helpers import CreateLockedFile
...
def start_postprocessing(IOW_ESM_ROOT, run_name, model, initial_start_date, end_date, task = "process_raw_output"):
    ...
    import socket
    job_script = "postprocess_job_"+socket.gethostname()+"."+str(os.getpid())
    command += "cat sbatch_header >> "+job_script+"; "
    command += "echo \"\" >> "+job_script+"; "
    command += "echo \"source ../../load_modules.sh\" >> "+job_script+"; "
    command += "echo \"./postprocess.sh " + IOW_ESM_ROOT+'/output/'+run_name+'/'+model + " " + str(initial_start_date) + " " + str(end_date) + "\" >> "+job_script+"; "
    command += "chmod u+x "+job_script+"; sbatch "+job_script
    
    # here could be the implementation for other queueing systems
    
    # lock the file until command is submitted
    lock_file_name = model_postprocess_dir+"/"+job_script
